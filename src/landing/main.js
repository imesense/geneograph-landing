...
document.addEventListener('click', event => {
  if (navToggle?.getAttribute('aria-expanded') === 'true' && !event.target.closest('[data-site-header]')) {
...
window.addEventListener('resize', () => {
  if (window.matchMedia('(min-width: 761px)').matches) setNavigationOpen(false);
});
...
document.querySelectorAll('[data-config-link]').forEach(placeholder => {
  ...
  if (/^https?:/i.test(url)) link.rel = 'noopener noreferrer';
...
form?.elements.email?.addEventListener('input', () => {
  if (form.elements.email.getAttribute('aria-invalid') === 'true') validateEmail();
});
